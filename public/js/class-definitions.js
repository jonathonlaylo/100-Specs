 ...
 *
 */
function favoritePlanet(yourFavPlanet){
  if(planets.indexOf(yourFavPlanet) > -1){
    var randomPlanet = planets[Math.floor(Math.random()*planets.length)];
    return `I'm from ${yourFavPlanet}, but I wish I could go to ${randomPlanet}`;
  } else {
  return `${yourFavPlanet} is not a planet!`;
  }
}
...
class Scientist extends Person {
  ...
  addDiscovery(disc){
    this.discoveries.push(disc);
      if(this.discoveries.indexOf(disc) === 0){
        return "I discovered " + this.discoveries[0] + ".";
      }if(this.discoveries.indexOf(disc) == 1){
        return "I discovered " + this.discoveries[0] + " and " + this.discoveries[1] + "." ;
      }if(this.discoveries.indexOf(disc) == 2){
...
var notWarmOrColdAnimal = new Animal('Aliean', 'Both').isWarmBlooded();
 ...
 *
 */
var streetDriving = new Vehicle('Honda', 'Civic').drive('Home');
var forwardDriving = new Vehicle('Toyota', 'Tacoma').drive(66);
...
var polygon = new Shape(1111).getType();
...
var closeAnOpenBox = new Box('Tacocat', true).openBox();
 ...
 *
 */
var farTooTallAStory = new House(2333).isATallStory(1);
var shortStory = new House(1).isATallStory(6666);
...
var porchLightsOff = new Lightbulb(false).flipSwitch('off');
...
var cookieMonsterBlocked = new Cookie('Not CoookieMonster cookie').swipedByCookieMonster('Not Monday');
 ...
 *
 */
var badForYou = new Meal(['chips', 'soda', 'drugs']).containsJunkFood();
var goodForYou = new Meal(['vegetables', 'unicorn', 'drugs']).containsJunkFood();
